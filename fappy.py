...
FLOOR = pygame.transform.scale2x(pygame.image.load(os.path.join('imgs', 'base.png')))
...
class Bird:
    ...
    def draw(self, win):
        ...
        new_rec = rotate_image.get_rect(center=self.img.get_rect(topleft=(self.x,self.y)).center)
        win.blit(rotate_image, new_rec.topleft)
...
class Pipe:
    ...
    def collide(self, bird):
        ...
        bott_mask = pygame.mask.from_surface(self.PIPE_BOTTOM)

        top_offset = (self.x - bird.x, self.top - round(bird.y))
        bottom_offset = (self.x - bird.x, self.bottom - round(bird.y))

        t_point = bird_mask.overlap(top_mask, top_offset)
        b_point = bird_mask.overlap(bott_mask, bottom_offset)
...
class Base:
    ...
    def draw(self, win):
        win.blit(FLOOR, (self.x1, self.y))
        win.blit(FLOOR, (self.x2, self.y))
